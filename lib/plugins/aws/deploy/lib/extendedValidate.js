  ...
  extendedValidate() {
    ...
    if (!_.isEmpty(this.serverless.service.functions) &&
      ...
      _.forEach(
        ...
        functionName => { // eslint-disable-line consistent-return
          ...
          if (!this.serverless.utils.fileExistsSync(artifactFilePath)) {
            missingFile = artifactFileName;
            return false;  // Early loop exit on error
          }
        }
      );
      if (missingFile) {
        return BbPromise.reject(new this.serverless.classes
          .Error(`No ${missingFile} file found in the package path you provided.`));
      }
